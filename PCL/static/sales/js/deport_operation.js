...
function update_return_rate()
{
    ...
    var product_id = $("#id_fp_item_many_to").children().first().val();
    if(!product_id){
    	product_id = $("#id_fp_item_chained").find(":selected").val();
    }

    if(!product_id){
    	console.log("\n\n Please select a product first");
    }
    var target_id = "id_return_rate"; 
    var ajax_url =  "/ajax_request/deport_return/";
	// console.log(v);
  ...
  $.get({
          "type"     : "GET",
          "data"	 : {'memo_no': memo_no, 'product_id': product_id},
          "url"      : ajax_url,
          "dataType" : "json",
          "cache"    : false,
